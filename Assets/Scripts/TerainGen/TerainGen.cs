   ...
   // float grassBaseHeight = 1;
    ...
    public Chunk ChunkColumnGen(Chunk chunk, int x, int z)
    {
        ...
        for (int y = chunk.pos.y - 8; y < chunk.pos.y + Chunk.chunkSize; y++)
        {
            ...
            {
                TerrainEdit.SetBlock(x, y, z, new Block(), chunk);
            }
            else if (y == dirtHeight && caveSize < caveChance) //Add caveSize < caveChance
            {
                TerrainEdit.SetBlock(x, y, z, new BlockGrass(), chunk);
            ...
            {
                TerrainEdit.SetBlock(x, y, z, new BlockDirt(), chunk);
            }
            else
            {
                TerrainEdit.SetBlock(x, y, z, new BlockAir(), chunk);
            }
    ...
    void CreateTree(int x, int y, int z, Chunk chunk)
    {
        ...
        for (int xi = -2; xi <= 2; xi++)
        {
            for (int yi = 4; yi <= 8; yi++)
            {
                ...
                {
                    TerrainEdit.SetBlock(x + xi, y + yi, z + zi, new BlockLeaves(), chunk, true);
                }
            }
        }
        ...
        {
            TerrainEdit.SetBlock(x, y + yt, z, new BlockWood(), chunk, true);
        }
    }
}
